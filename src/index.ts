...
const checkVersion = async () => {
  ...
  try {
    // Short timeout to avoid blocking startup in restricted networks
...
program
  ...
  .option(
    "--env <ENV_PATH>",
    "Path to .env file, default to current working folder (See .env.example)"
  )
  .option(
    "--config <CONFIG_PATH>",
    "Path to config file (See config.example.json)"
  )
  .option(
    "--user <USER>",
    "Monitor & download live Spaces from users, separate by comma (,)"
  )
  .option("--id <SPACE_ID>", "Monitor & download live Space with its id")
  .option(
    "-surl, --space-url <SPACE_URL>",
    "Monitor & download live Space with its URL"
  )
  ...
  .option(
    "--auth-token <TOKEN>",
    "Twitter cookie auth_token, overrides .env at startup"
  )
  .option(
    "--csrf-token <TOKEN>",
    "Twitter cookie ct0 (CSRF), overrides .env at startup"
  )
...
program.action(async (args, cmd: Command) => {
  ...
  // initialize token manager (CLI > tokens.json > .env)
  tokenManager.init({
  ...
  const mask = (s?: string) => (s ? `${String(s).substring(0, 16)}****` : "");
  ...
  configManager.update({
    ...
    skipDownloadAudio:
      args.skipDownloadAudio ?? configManager.config.skipDownloadAudio,
    skipDownloadCaption:
      args.skipDownloadCaption ?? configManager.config.skipDownloadCaption,
  });
  ...
  const usernames = [
    ...new Set(
      (user || "")
        .split(",")
        .concat(
          (configManager.config.users || []).map((v) =>
